...
- 支持 `\overset{上}{基础}` 上堆叠
- 支持 `\underset{下}{基础}` 下堆叠
- 支持 `\stackrel{上}{基础}` 堆叠（同 overset）
- 验证复杂内容和嵌套场景
...
#### 2. SimpleFormulaTest.kt (43 个测试) ⭐⭐ 中等
...
#### 6. EnvironmentExtensionTest.kt (15 个测试) ⭐⭐⭐ 复杂
...
-----|
| BasicSymbolsTest | 基础符号 | 25 | ⭐ 简单 |
| SimpleFormulaTest | 简单公式 | 43 | ⭐⭐ 中等 |
| ComplexStructureTest | 复杂结构 | 45 | ⭐⭐⭐ 复杂 |
| RealWorldFormulaTest | 真实公式 | 14 | ⭐⭐⭐⭐ 高级 |
| EdgeCasesTest | 边界情况 | 24 | ⭐⭐⭐ 复杂 |
| EnvironmentExtensionTest | 环境扩展 | 15 | ⭐⭐⭐ 复杂 |
...
2. **全面覆盖** - 266 个测试用例覆盖所有主要 LaTeX 语法
   - 基础符号: 25 个测试
   - 简单公式: 43 个测试（含二项式系数、颜色、堆叠）
...
4. **关键功能支持** - 覆盖重要LaTeX特性
   - ✅ 环境扩展 `split, multline, eqnarray, subequations` (15个测试)
