...
#### 6. EnvironmentExtensionTest.kt (15 个测试) ⭐⭐⭐ 复杂 ⭐新增

环境扩展测试，验证新增的LaTeX环境支持。
...
**适用场景**: 学术论文、多行方程、方程组编号

---

### 工具类测试 (3个文件, 27个测试)

#### 7. SymbolMapTest.kt (26 个测试)
...
| EnvironmentExtensionTest | 环境扩展 | 15 | ⭐⭐⭐ 复杂 | ⭐新增 |
...
**Split 环境 (3个测试)**
- 支持 `\begin{split}...\end{split}` 分割环境
- 用于单个方程内的多行分割,常用于 equation 环境内
- 支持对齐符号 `&` 和换行符 `\\`

**Multline 环境 (3个测试)**
- 支持 `\begin{multline}...\end{multline}` 多行环境
- 第一行左对齐,最后一行右对齐,中间行居中
- 适用于需要多行显示的单个方程

**Eqnarray 环境 (3个测试)**
- 支持 `\begin{eqnarray}...\end{eqnarray}` 方程数组
- 旧式语法,三列结构(左边、关系符、右边)
- 与 align 环境类似,但语法稍有不同

**Subequations 环境 (3个测试)**
- 支持 `\begin{subequations}...\end{subequations}` 子方程
- 用于对一组相关方程进行编号(如 1a, 1b, 1c)
- 可以包含其他环境(equation, align等)
...
./gradlew :latex-parser:test --tests "EnvironmentExtensionTest"  # 环境扩展 ⭐新增
...
1. **职责分离** - 9 个测试文件按职责清晰划分，避免单文件过大
   - 解析器测试 (6个): 基础→简单→复杂→真实→边界→环境
   - 工具类测试 (3个): 符号映射、词法分析、访问者模式

2. **全面覆盖** - 261 个测试用例覆盖所有主要 LaTeX 语法
   - 基础符号: 24 个测试
   - 简单公式: 35 个测试（含二项式系数）
...
4. **新功能支持** - 最新版本补充关键功能
   - ✅ 环境扩展 `split, multline, eqnarray, subequations` (15个测试) ⭐新增
